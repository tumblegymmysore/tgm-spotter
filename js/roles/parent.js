// js/roles/parent.js (v58 - Visuals Restored + Validation Fixed)
...
// --- 1. INTAKE FORM (With Validation Fix) ---
export async function handleIntakeSubmit(e) {
    e.preventDefault();
    const btn = document.getElementById('btn-submit');
    const originalText = btn.innerText;
    
    // 1. Capture & Sanitize Inputs
    const email = document.getElementById('email').value.trim();
    const phone = document.getElementById('phone').value.trim().replace(/\D/g, ''); // Remove non-digits
    const altPhone = document.getElementById('alt_phone').value.trim().replace(/\D/g, ''); // Remove non-digits

    // 2. STRICT VALIDATION (Fixes DB Constraint Error)
    if (phone.length !== 10) {
        alert("Primary Mobile Number must be exactly 10 digits.");
        return;
    }
    ...
    try {
        ...
        // Manually set text in case utils default kicks in
...
// --- 2. PARENT DASHBOARD (Visuals Restored) ---
export async function loadParentDashboard(email) {
    ...
    for (const child of data) {
        ...
        // --- A. TRIAL COMPLETED (Restored Visuals) ---
        if (child.status === 'Trial Completed') {
            statusBadge = 'Assessment Ready';
            statusColor = 'bg-blue-100 text-blue-700';
            const rec = child.recommended_batch || 'Standard';
            
            // This restores the Blue Box with Checkmark from your screenshot
            actionArea = `
                <div class="bg-blue-50 p-4 rounded-xl mb-4 border border-blue-100 flex items-start gap-3">
                    <div class="bg-blue-600 text-white rounded-full w-6 h-6 flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
                        <i class="fas fa-check text-xs"></i>
                    </div>
                    <div>
                        <h4 class="font-bold text-blue-900 text-sm">Trial Successful!</h4>
                        <p class="text-xs text-blue-700 mt-1">Trainer recommends: <strong>${rec}</strong></p>
                    </div>
                </div>
                <button onclick="window.openRegistrationModal('${leadString}', false)" class="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-200 hover:bg-blue-700 transition">
                    Proceed to Registration
                </button>`;
        }
        
        // --- B. REQUESTED APPROVAL ---
        else if (child.status === 'Enrollment Requested') {
            statusBadge = 'Pending Approval';
            statusColor = 'bg-orange-100 text-orange-700';
            actionArea = `
                <div class="bg-orange-50 p-4 rounded-xl mb-4 border border-orange-100 flex items-start gap-3">
                    <div class="bg-orange-500 text-white rounded-full w-6 h-6 flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
                        <i class="fas fa-clock text-xs"></i>
                    </div>
                    <div>
                        <h4 class="font-bold text-orange-900 text-sm">Request Sent</h4>
                        <p class="text-xs text-orange-800 mt-1">Admin is verifying batch availability.</p>
                    </div>
                </div>
                <button disabled class="w-full bg-orange-100 text-orange-400 font-bold py-3 rounded-xl cursor-not-allowed">Waiting for Admin...</button>`;
        }

        // --- C. APPROVED (READY TO PAY) ---
        else if (child.status === 'Ready to Pay') {
            statusBadge = 'Approved';
            statusColor = 'bg-green-100 text-green-700';
            actionArea = `
                <div class="bg-green-50 p-4 rounded-xl mb-4 border border-green-100 flex items-start gap-3">
                    <div class="bg-green-600 text-white rounded-full w-6 h-6 flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
                        <i class="fas fa-check-double text-xs"></i>
                    </div>
                    <div>
                        <h4 class="font-bold text-green-900 text-sm">Admission Approved!</h4>
                        <p class="text-xs text-green-800 mt-1"><strong>${child.final_batch}</strong><br>Fee: ₹${child.final_price}</p>
                    </div>
                </div>
                <button onclick="window.openRegistrationModal('${leadString}', false)" class="w-full bg-green-600 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-green-200 hover:bg-green-700 animate-pulse">
                    Pay Now & Enroll
                </button>`;
        }
        ...
        // --- D. PAYMENT VERIFICATION ---
             ...
             actionArea = `
                <div class="text-center p-4 bg-purple-50 rounded-xl border border-purple-100">
                    <p class="text-xs font-bold text-purple-700 mb-2">Payment Receipt Uploaded</p>
                    <button disabled class="bg-white text-purple-400 text-xs font-bold py-2 px-4 rounded-lg border border-purple-100">Processing...</button>
                </div>`;
        } 
        ...
        // --- E. ENROLLED ---
             ...
             actionArea = `
                <div class="flex items-center gap-2 mb-4 text-emerald-800 text-xs font-bold bg-emerald-50 px-3 py-1.5 rounded-lg w-fit border border-emerald-100">
                    <span class="w-2 h-2 bg-emerald-500 rounded-full"></span> Active
                </div>
                <button onclick="window.openRegistrationModal('${leadString}', true)" class="w-full border-2 border-emerald-600 text-emerald-700 font-bold py-3 rounded-xl hover:bg-emerald-50 transition">
                    Renew Membership
                </button>`;
        }
        ...
        // --- F. ON HOLD ---
            ...
            actionArea = `
                <div class="text-xs text-orange-800 bg-orange-50 p-3 rounded-lg mb-3 border border-orange-100">Follow-up: <strong>${fDate}</strong></div>
                <button onclick="window.openRegistrationModal('${leadString}', false)" class="w-full bg-orange-500 text-white font-bold py-3 rounded-xl shadow-md hover:bg-orange-600">Resume Registration</button>`;
        }
            ...
            <div class="relative rounded-3xl p-6 shadow-sm border border-slate-100 bg-white mb-4 hover:shadow-md transition-all duration-300">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex gap-4 items-center">
                        <div class="w-12 h-12 rounded-2xl bg-slate-100 text-slate-500 flex items-center justify-center font-bold text-lg">
                    ...
                    <span class="${statusColor} text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wide">${statusBadge}</span>
                </div>
                ${actionArea}
                <div class="flex gap-3 mt-4 pt-4 border-t border-slate-50">
                    <button onclick="window.openParentChat('${leadString}')" class="flex-1 text-xs font-bold text-slate-500 hover:text-blue-600 relative py-2 rounded-lg hover:bg-slate-50 transition">
                        <i class="fas fa-comment-alt mr-2"></i>Chat with Coach ${msgBadge}
                    </button>
                    <button onclick="window.openEditModal('${leadString}')" class="w-10 text-xs font-bold text-slate-500 hover:text-blue-600 py-2 rounded-lg hover:bg-slate-50 transition">
...
// --- 3. SMART REGISTRATION LOGIC (Logic Preserved) ---
...
export function checkApprovalRequirement() {
    ...
    // Logic: Changed from recommended OR selected "Other" OR Kid moving to Morning
...
// --- 4. SUBMIT (Updated Success Messages) ---
